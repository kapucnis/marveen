...
window._i18n.en = {
  ...
  'network.page_title':          'Network',
  'network.page_subtitle':       'Camera reachability check on the local network',
  'network.ip_placeholder':      '192.168.x.x',
  'network.check_btn':           'Check',
  'network.checking':            'Checking...',
  'network.hint':                'Reachability test: it shows whether the device responds on the network, not whether it is working correctly.',
  'network.result.isapi':        'Online -- Hikvision web service responds ({ms}ms)',
  'network.result.isapi_noms':   'Online -- Hikvision web service responds',
  'network.result.online_no_web':'Reachable, but the web interface does not respond',
  'network.result.offline':      'Not reachable',
  'network.result.busy':         'A check is already running, try again in a moment.',
  'network.result.invalid_ip':   'Invalid IP address.',
  ...
  'updates.toast.fork_short':    'This install is a divergent fork; the one-click update cannot apply here. Details below.',
  'updates.fork.title':          'Divergent fork, one-click update unavailable',
  'updates.fork.detail':         'The current branch ({branch}) is not on origin, so there is no upstream ref for a fast-forward update. This install is a divergent fork. Update it through the fleet-update-watch process (the categorised SAFE install plan, then the Yoda/EliteAI merge-build-deploy round), not a one-click pull.',
